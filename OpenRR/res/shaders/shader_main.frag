...
{% if textures %}
		colour = texture2D(diffuseTexture, texCoord) * vec4(ambient + diffuse + specular, 1.0);;
{% endif %}
...
{% if colourMaterial %}
		...
		colour = vec4(colour_amb.rgb + colour_dif.rgb + colour_spe.rgb + colour_emi.rgb, min(colour_dif.a, colour_emi.a));
{% endif %}
